...
import { signup as signupApi, oauthStartUrl } from "@/services/auth";
...
export function SignupForm({ className, ...props }: Props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Account created", description: res.user.name });
      navigate("/dashboard");
    } catch (err: any) {
      ...
      <div className="grid gap-6">
        <div className="grid gap-3">
          ...
          <div className="relative">
            ...
            <Input id="name" placeholder="Jane Doe" required value={name} onChange={(e) => setName(e.target.value)} className="pl-9" />
          </div>
        </div>
        <div className="grid gap-3">
